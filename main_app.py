...
def in_workshop():
    ...
    st.sidebar.write('事業側の利益:')
    profit = state.saved_proposal[st.sidebar.number]['profit']
    st.sidebar.write(f'{profit[0]:.02f} ~ {profit[1]:.02f} 兆円')
    ...
    st.divider()
    st.subheader('提案と提案の記録')
    st.warning('提案が受け入れられたかどうかに関わらず、提案が行われたら必ず記録してください。')

    role = st.radio('提案者を選択してください', ('事業側', '住民側'))
    the_other_role = '住民側' if role == '事業側' else '事業側'
    st.write(f'{role}が提案している。')
    ...
    st.write(
        f'事業側の利益：{state.current_proposal[0][0]:.02f} ~ {state.current_proposal[0][1]:.02f} 兆円')
    st.write(
